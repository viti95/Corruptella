...
function corClass.dump(filename)
	dumpMEM(filename .. "_1", ":cartslot1:rom", 8)
end

function corClass.restore(filename)
	restoreMEM(filename .. "_1", ":cartslot1:rom", 8)
end

function corClass.fuckRAM(mode)
	if (mode == 0) then
		fuckADDR(":maincpu", "program", 1, 8, 32)
	end
...
function corClass.fuckROM(mode)
	if (mode == 0) then
		fuckMEM(":cartslot1:rom", 8, 32)
	end

	if (mode == 1) then
		fuckMEM(":cartslot1:rom", 8, 1)
	end
end
